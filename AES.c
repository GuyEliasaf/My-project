...
void InvSubBytes(char state[4][4])
{
	int pos; 
	int i, j;
	printf("%d", getSBoxInvert(state[0][0]));
	for (i = 0; i <4; i++)
	{
		for (j = 0; j < 4; j++)
		{
			pos = getSBoxInvert(state[i][j]);
			state[i][j] = pos;
		}
	}
}

void ShiftRow(char input[4][4])
{
	char output[4][4] = {
...
static void AddRoundKey(char round, char state[4][4], char RoundKey[16])
{
	int i, j;
	for (i = 0; i < 4; i++)
	{
...
static void Cipher(char state[4][4], char RoundKey[16])
{
	int round = 0;
...
int main() {
	...
	char state[4][4] = { "aaaa","bbbb","cccc","dddd" };
